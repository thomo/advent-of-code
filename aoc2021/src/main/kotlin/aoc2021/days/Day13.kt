...
class Day13 : DayXX {
	...
	private fun foldVertical(points: Set<Pair<Int, Int>>, pos: Int) =
		swapCoords(foldHorizontal(swapCoords(points), pos))

	private fun swapCoords(points: Set<Pair<Int, Int>>) = points.map { Pair(it.second, it.first) }.toSet()

	private fun foldField(points: Set<Pair<Int, Int>>, instr: Pair<String, Int>) =
		if (instr.first == "x") foldVertical(points, instr.second) else foldHorizontal(points, instr.second)
