...
class Day14 : DayX {
    ...
    fun countChars(map: Map<String, Long>, first: Char, last: Char) =
            map.entries
                    ...
                    .fold(mapOf(first to 1L, last to 1L)) { acc, e -> acc + Pair(e.first, acc.getOrDefault(e.first, 0L) + e.second) }
    ...
    private fun generateExtRules(rules: Map<String, String>) = rules.mapValues { listOf(it.key[0] + it.value, it.value + it.key[1]) }
