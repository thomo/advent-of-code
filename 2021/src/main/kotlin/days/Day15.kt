...
class Day15 : DayX {
    ...
    private fun getHorOrVar(p: Day15.MyPoint) =
            listOf(Day15.MyPoint(p.x - 1, p.y), Day15.MyPoint(p.x + 1, p.y), Day15.MyPoint(p.x, p.y - 1), MyPoint(p.x, p.y + 1))
    ...
    fun linePlusOne(line: String) = line.toCharArray().toList().map { c -> c.toString().toInt() % 9 + 1 }.joinToString("")
