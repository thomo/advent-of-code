...
class Day16 {
    ...
    private fun checkRules(validTickets: MutableList<List<Int>>, rules: List<Day16.Rule>) {
        rules.forEach { rule ->
            if (!validTickets.all{ t -> rule.match(t[rule.index])}) println("Fail: $rule")
        }
    }

    private fun findSingleOpenRule(matchingRules: MutableList<List<Day16.Rule>>): Boolean {
