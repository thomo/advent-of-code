...
class Day20 {
    class Tile(val id: Int, tileLines: List<String>) {
        ...
        fun fitBelow(tile: Tile): Boolean = top() == tile.bottom()
        fun fitRight(tile: Tile): Boolean = left() == tile.right()
...
//        print("1: #    ##    ##    ###")
        val result =
            line[0] == '#' && line[5] == '#' && line[6] == '#' && line[11] == '#' && line[12] == '#' && line[17] == '#' && line[18] == '#' && line[19] == '#'
        //return if (result) {println(" -> 1"); true} else {println(" -> 0"); false}
        return result
    }
...
//        return if (result) {println(" -> 1"); true} else {println(" -> 0"); false}
    ...
    private fun buildImage(tiles: List<Tile>, dim: Int): List<String> {
    ...
    private fun arrangeTiles(dim: Int, arranged: List<Tile>, tiles: List<Tile>): List<Tile> {
