...
class Day19 {
    ...
    private fun generateLang(startRule: String, rules: List<Day19.Rule>): List<String> {
        return rules.first { it.left == startRule }.result().flatMap{ start -> replace(start, rules).map { it.joinToString("") }}
    }

    private fun replace(start: List<String>, rules: List<Day19.Rule>): List<List<String>> {
